...
### Changed
- Marked messages are no longer shown with reverse video, which was easy to confuse with the cursor row. They are now indicated by a red `𝐌` symbol in the flags column alongside the unread/replied/flagged markers, and the symbol is configurable via `display.flag_marked`. The `colors.marked` option no longer has any effect (it is still accepted in existing configs for compatibility).
...
- Fixed compose text-alias Tab completion moving the cursor to the top of the message and delaying the visible expansion until a later cursor movement. Alias expansion now preserves the cursor position and repaints the edited line immediately.
